...
**Status: ✅ SHIPPED.** Gate-4 triple review came back clean — **0 BLOCKER, 0 MAJOR**; 8 MINOR (cheap doc/comment/gofmt nits, all fixed in `fa2c3ac`) + 4 security findings all verified down to *non-issue* (the trust boundary is intact: signing key stays isolated, no silent unsigned-write path, fail-safe read-only fallback). Fast-forward-merged to main and pushed: **`f1636d8..40159e7` → origin/main** (2026-06-04). 22 commits. Your tester can install now. The two deferred MINORs (G2 init-mkdir no-op; G3 test-table style) are correct-as-is polish — listed here only for completeness, no action needed. Now moving to the v1.2 R2–R7 redactor roadmap.
