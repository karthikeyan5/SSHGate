...
**Architecture:** Three Unix trust domains — karthi (MCP + SSH client), signer (separate user owning master Ed25519 signing key + Telegram bot token), Telegram (Karthi's phone for `from.id`-authenticated approval). MCP↔signer over Unix socket; signer↔Telegram via direct bot API DM. gate binary on each remote server enforces the gate via OpenSSH `command="..."` forcing and Ed25519 signature verification. Backend interface in signer is swappable (Telegram now, hosted server in v2).
...
│   │   └── audit.go               # AuditEvent struct (shared between signer + MCP)
│   ├── gate/
│   │   ├── cmd/gate/main.go    # Entry point; reads SSH_ORIGINAL_COMMAND
...
│   │   └── doc.go
│   ├── signer/
│   │   ├── cmd/signer/main.go  # Entry; loads config, builds backend, runs daemon
...
│   │   ├── sign/
│   │   │   ├── client.go          # Talks to signer socket; encodes/decodes JSON-Lines
│   │   │   └── client_test.go     # Against a fake signer socket
│   │   ├── registry/
...
│       └── SKILL.md
├── scripts/
│   ├── install.sh                 # Called by /sshgate:setup; creates signer user, writes systemd unit
│   ├── uninstall.sh
│   └── create-signer-user.sh
├── tests/
│   ├── integration/
│   │   ├── docker-compose.yml      # linuxserver/openssh-server target
│   │   ├── e2e_test.go             # Build-time: compose up, deploy gate, run end-to-end
│   │   └── helpers_test.go
│   └── testdata/
...
- `common/` has zero inbound deps — both gate and signer+MCP import it; classifier + payload format live here so there is exactly one source of truth.
- `gate/` is a single tiny binary; flat structure.
- `signer/` has a `backend/` sub-package because the Backend interface is the swap-point for v2; isolating it makes the interface obvious.
...
**Parallelism:** tasks within a phase are sequential by default unless explicitly marked `[parallel]`. Phase 1's `common/`, Phase 1's gate, and Phase 1's signer can be parallelized once common/ ships.
...
- [ ] **Step 4:** Write `Makefile` with targets: `build` (`go build ./cmd/...`), `test` (`go test -race ./...`), `vet` (`go vet ./...`), `clean` (`rm -rf bin/`), `gate-linux` (cross-compile for remote deploy)
...
Goal: end-to-end SSH→gate→exec for read commands. signer uses StubBackend (always denies); MCP wires SSH client + sign client + classifier. By end of phase: `sshgate.run("test-server", "df -h")` returns disk usage; `sshgate.run("test-server", "rm /tmp/x")` returns "denied by operator."
...
### Task 1.2: common/payload — SSHGATE_SIG payload format
...
// SSHGATE_SIG:<sig>:<payload-b64> wire format defined in the spec.
...
// EncodeSigned returns "SSHGATE_SIG:<sigB64>:<payloadB64>"
...
// IsSigned reports whether s starts with the SSHGATE_SIG: prefix
...
### Task 1.3 [parallel with 1.4, 1.5]: gate binary

**Spec:** §"Command Classification" §"Signature Scheme" §"Transport"
**Files:** `src/gate/cmd/sshgate-gate/main.go`, `src/gate/verify.go`, `src/gate/verify_test.go`, `src/gate/executor.go`, `src/gate/executor_test.go`, `src/gate/doc.go`

**Interface contracts:**
```go
// In src/gate/verify.go:
// VerifySigned checks the SSHGATE_SIG prefix matches the inner cmd,
...
1. Read `SSH_ORIGINAL_COMMAND` from env. If empty → exit 0 with "SSHGATE_OK" (the test-setup probe per spec §"Installation" step 6).
2. Load `~/.sshgate-gate/gate.pub` (mode check: 0644 expected, refuse if more permissive).
...
5. If `innerCmd == "SSHGATE_REVOKE"` and signed → run revoke (defer to phase 4); for phase 1 stub it.
6. If `innerCmd == "SSHGATE_UPDATE …"` and signed → deferred to v1.1; stub.
...
- [ ] **Step 7:** Wire `main.go` per logic above. Stderr is the operator channel; stdout is the executed command's stdout (per daemon.md §6.3 and cli.md §3 — though gate is sub-binary, the stdio discipline applies).
- [ ] **Step 8:** Build + smoke: `go build -o /tmp/gate ./src/gate/cmd/sshgate-gate && SSH_ORIGINAL_COMMAND="ls /" /tmp/gate` lists root.
- [ ] **Step 9:** Run `go test -race ./src/gate/...` clean.
- [ ] **Step 10:** Commit: `feat(gate): SSH command gate with read/write classification and Ed25519 verify (task 1.3)`

### Task 1.4 [parallel with 1.3, 1.5]: signer — keystore, socket, daemon core (StubBackend phase)

**Spec:** §"Components — signer daemon"
**Files:** `src/signer/keystore.go`, `keystore_test.go`, `src/signer/socket.go`, `socket_test.go`, `src/signer/audit.go`, `audit_test.go`, `src/signer/daemon.go`, `daemon_test.go`, `src/signer/backend/backend.go`, `backend/stub.go`, `backend/stub_test.go`, `backend/mock.go`, `backend/doc.go`, `src/signer/cmd/sshgate-signer-telegram/main.go`, `src/signer/doc.go`

**Interface contracts:**
```go
// src/signer/backend/backend.go
package backend
...
- Response: `{"request_id":"r_xxx","status":"approved","signatures":[{"cmd":"b","sig":"SSHGATE_SIG:..."}]}\n` or `{"request_id":"r_xxx","status":"denied"}\n` or `{"request_id":"r_xxx","status":"timeout"}\n`
...
- Lives at `/var/lib/sshgatesigner/log/approvals.log` in production; configurable.

**main.go logic:**
1. Parse `--config` flag (default `/etc/signer/config.toml`).
2. Validate: running as the `sshgatesigner` user; refuse to start otherwise (per daemon.md §1.2 fail-fast).
3. Acquire `flock` on `/run/sshgatesigner/sock.lock` (daemon.md §3.1 — singleton). Fail fast if held.
...
7. Bind Unix socket at `/run/sshgatesigner/sock` mode 0660, group `sshgatesigner` (so karthi user in `sshgatesigner` group can connect).
...
- [ ] **Step 10:** Write `daemon_test.go` with MockBackend: full sign request → approve → response carries signatures verifiable by gate; deny path; timeout path; concurrent requests don't cross results (one channel per request_id).
- [ ] **Step 11:** RED → implement `daemon.go` → GREEN.
- [ ] **Step 12:** Wire `cmd/signer/main.go` per logic above. Use `signal.NotifyContext` for graceful shutdown (daemon.md §2.6).
- [ ] **Step 13:** `go test -race ./src/signer/...` clean.
- [ ] **Step 14:** Add `goleak.VerifyTestMain` to `signer/daemon_test.go` (go.md §4.11).
- [ ] **Step 15:** Commit: `feat(signer): daemon, socket, keystore, audit, stub backend (task 1.4)`
...
// Sign sends an approval request to signer and returns the signed wire
...
- Local classify:
  ...
  - Write → call `sign.Client.Sign` for a single-cmd request; on approval, SSH with the signed `SSHGATE_SIG:...:... <inner-cmd>` prefix; return output. On denial/timeout → return a structured MCP tool error with the reason.
...
- [ ] **Step 5:** GREEN.
- [ ] **Step 6:** Write `sign/client_test.go` against an in-process fake signer socket (use `net.Pipe()` or a real Unix socket in `t.TempDir()`): approval path, denial path, timeout path, unreachable path.
...
1. **Read path** — boot Docker sshd, generate fresh sshgate_ed25519 key, generate fresh gate signing key, copy gate binary into the container's user home, install authorized_keys with `command="..."` forcing, start signer with StubBackend in a goroutine, start MCP, call `sshgate.run("test", "df -h")` → expect 0 exit + non-empty stdout.
2. **Write denied** — same setup; call `sshgate.run("test", "rm /tmp/x")` → expect tool error with "denied by operator" (StubBackend denies).
3. **Direct SSH bypass attempt** — try to SSH into the container WITH the dedicated key but WITHOUT the gate signature prefix → confirm classifier on the remote denies writes.
...
**Spec:** §"Components — signer-bot (Telegram bot, DM-only)"
**Files:** `src/signer/backend/telegram.go`, `telegram_test.go`
...
**Approval message text:** see spec §"signer-bot — Approval message shape".
...
- [ ] **Step 4:** Commit: `feat(signer/backend): TelegramBackend with inline-keyboard approvals (task 2.1)`

### Task 2.2: ChatStore — persistent capture of Karthi's DM chat_id

**Files:** `src/signer/backend/chatstore.go`, `chatstore_test.go`
...
type FileChatStore struct { Path string }  // JSON file at /var/lib/sshgatesigner/config/peer.json
```
...
- [ ] **Step 4:** Commit: `feat(signer/backend): ChatStore for DM capture (task 2.2)`
...
**Files:** `commands/setup.md`, `scripts/install.sh`, `scripts/create-signer-user.sh`, `docs/install-step-by-step.md`
...
- Steps it walks:
  ...
  2. `go build ./cmd/...` to produce `bin/sshgate-mcp`, `bin/sshgate-signer-telegram`, `bin/sshgate-gate-linux-amd64`.
  3. Prompt Karthi to run `sudo scripts/create-signer-user.sh` (one-time, interactive).
  4. Generate keys (`bin/sshgate-signer-telegram --init`) — writes `/var/lib/sshgatesigner/keys/gate.{key,pub}` (mode 0600/0644) and `~/.config/sshgate/ssh/sshgate_ed25519` (mode 0600). Refuse to overwrite existing.
  5. Prompt Karthi: paste your @BotFather bot token. Save to `/var/lib/sshgatesigner/tokens/telegram.token`.
  6. Install systemd unit (`scripts/install.sh`) — `User=signer`, `ExecStart=/usr/local/bin/sshgate-signer-telegram`, `Restart=always`, `RestartSec=10`.
  7. Start signer: `sudo systemctl enable --now sshgate-signer-telegram`.
...
- [ ] **Step 2:** Write `scripts/create-signer-user.sh` — `useradd -r -s /usr/sbin/nologin sshgatesigner && usermod -a -G sshgatesigner $SUDO_USER && mkdir -p /var/lib/sshgatesigner/{keys,tokens,config,log} && chown -R sshgatesigner:sshgatesigner /var/lib/sshgatesigner && chmod 0750 /var/lib/sshgatesigner`.
- [ ] **Step 3:** Write `scripts/install.sh` — copies binaries to `/usr/local/bin/`, writes systemd unit at `/etc/systemd/system/sshgate-signer-telegram.service`, `systemctl daemon-reload`.
...
Uses a fake Telegram server (httptest) and asserts the full path: MCP receives `run_batch` → calls sign client → signer (with TelegramBackend pointed at fake Telegram) → fake Telegram delivers a programmatic "approve" callback → signer signs → MCP SSH-executes → outputs collected.
...
3. Connect to the remote using Karthi's normal SSH key (interactive — assumes `~/.ssh/id_ed25519` works, or uses ssh-agent). This is the bootstrap leg; once gate is installed, future connections use the dedicated key.
4. `mkdir -p ~/.sshgate-gate && chmod 700 ~/.sshgate-gate`
5. SCP-equivalent: upload `bin/sshgate-gate-linux-amd64` → `~/.sshgate-gate/gate`, `chmod 755`.
6. Upload `gate.pub` (the signing public key) → `~/.sshgate-gate/gate.pub`, `chmod 644`.
7. Rewrite `~/.ssh/authorized_keys`: locate any existing line for the SSHGate pubkey (by comment `sshgate@laptop`), replace with the restricted line `command="~/.sshgate-gate/gate",no-port-forwarding,no-X11-forwarding,no-agent-forwarding <key>`. If no existing line, append.
8. Verify by reconnecting with the SSHGate key + sending `echo SSHGATE_OK` → expect that string back.
...
- [ ] **Step 1:** Write `add_server_test.go` against Docker sshd: success path; rollback on gate copy fail; rollback on authorized_keys verify fail; idempotency (re-add same alias should NOT duplicate authorized_keys lines).
- [ ] **Step 2:** RED → implement → GREEN.
- [ ] **Step 3:** Commit: `feat(mcp/tools): add_server auto-setup with rollback (task 3.1)`

### Task 3.2: gate cross-compile target + Makefile

- [ ] **Step 1:** Makefile target `gate-linux-amd64`: `CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags='-s -w' -o bin/sshgate-gate-linux-amd64 ./src/gate/cmd/sshgate-gate`
- [ ] **Step 2:** Verify the binary is statically linked: `file bin/sshgate-gate-linux-amd64` shows "statically linked" / "no shared libs".
- [ ] **Step 3:** Add to `make build` umbrella target.
- [ ] **Step 4:** Commit: `build: cross-compile gate for linux/amd64 (task 3.2)`
...
Boots a FRESH container (no authorized_keys preinstalled), calls `sshgate.add_server`, verifies the auto-setup worked end-to-end including reconnect and `echo SSHGATE_OK`.
...
- [ ] **Step 2:** `status` returns a health object: `{signer_socket_reachable: bool, servers: [{alias, reachable: bool, ping_ms}]}`.
- [ ] **Step 3:** Tests + impl + commit per pattern.

### Task 4.2: revoke_server tool + SSHGATE_REVOKE handler in gate

**Files:** `src/mcp/tools/revoke_server.go`, `revoke_server_test.go`, update `src/gate/cmd/sshgate-gate/main.go` to handle `SSHGATE_REVOKE` (signed).

revoke flow:
1. MCP signs a `SSHGATE_REVOKE` command via signer.
2. SSH into remote with signed command.
3. gate handles SSHGATE_REVOKE: deletes its own authorized_keys line, removes `~/.sshgate-gate/`, exits 0.
...
- LLM command explainer: signer posts a short "what does each command do" message alongside the approval prompt. Implementation: optional `LLMExplainer` field on `TelegramBackend`; when set, call it before sending the message; include explanation as a separate line per command.
- macOS desktop support: add `darwin/amd64` and `darwin/arm64` targets in Makefile; ensure no Linux-specific syscalls in non-gate code (gate stays Linux-only since remotes are Linux); cross-build the MCP and signer; integration test runs in CI on macOS image (defer — we can't test without a Mac, but we can confirm build).
- Automated signer user provisioning: `scripts/install.sh` handles `useradd` automatically rather than asking Karthi to run a separate script. Idempotent: skip if user exists.
...
- `src/signer-server/` directory + go module entry point.
...
- HostedServerBackend implementation in signer so the swap is one-line.
- Web UI: minimal — static HTML + a couple of htmx endpoints. No React.
- Deploy script: `cmd/sshgate-signer-server/deploy.sh` with documented VPS setup.
