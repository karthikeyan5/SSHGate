...
// TestRunReadOnlyMode covers the tier-1 install where velgate.pub is
...
// executor_test.
//
// The pubkey-missing branch only triggers if velgate's pubKeyPath()
...
// "velgate.pub" — so the read-only branch is exactly what we hit.
func TestRunReadOnlyMode(t *testing.T) {
	// Sanity: ensure the test binary's sibling velgate.pub really is
	...
	pub := filepath.Join(filepath.Dir(exe), "velgate.pub")
	if _, err := os.Stat(pub); err == nil {
		t.Skipf("velgate.pub present at %s; skipping read-only test", pub)
	}
	...
	t.Run("signed command exits 77 with descriptive stderr", func(t *testing.T) {
		...
		withEnv(t, "SSH_ORIGINAL_COMMAND", "VELGATE_SIG:abc:0:0:dummy ls")
		var code int
	...
	t.Run("empty command is the probe path and prints VELGATE_OK", func(t *testing.T) {
		...
		if !bytes.Contains(buf.Bytes(), []byte("VELGATE_OK")) {
			t.Errorf("stdout = %q, want VELGATE_OK", buf.String())
		}
	})
}
