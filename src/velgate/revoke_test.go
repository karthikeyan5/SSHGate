...
// seedVelgateDir creates home/.velgate with a couple of placeholder files.
func seedVelgateDir(t *testing.T, home string) string {
	t.Helper()
	dir := filepath.Join(home, ".velgate")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		t.Fatalf("mkdir .velgate: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "velgate"), []byte("binary"), 0o755); err != nil {
		t.Fatalf("seed binary: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "velgate.pub"), []byte("pubkey"), 0o644); err != nil {
...
// (typically <t.TempDir>/.velgate/velgate).
...
func TestRevoke_RemovesMatchingLineAndDir(t *testing.T) {
	home := t.TempDir()
	binPath := filepath.Join(home, ".velgate", "velgate")
	...
	velgateDir := seedVelgateDir(t, home)

	res, err := velgate.Revoke(home, velgateDir, binPath)
	if err != nil {
	...
	if !res.VelgateDirRemoved {
		t.Errorf("VelgateDirRemoved = false; want true")
	}
	if res.BackupPath == "" {
	...
	// velgate dir gone.
	if _, err := os.Stat(velgateDir); !os.IsNotExist(err) {
		t.Errorf("velgate dir still present: %v", err)
	}
}
...
func TestRevoke_NoMatchingLineLeavesFileAlone(t *testing.T) {
	...
	if !res.VelgateDirRemoved {
		t.Errorf("VelgateDirRemoved = false; want true (dir still exists)")
	}
}
...
func TestRevoke_AlreadyAbsentVelgateDirNotError(t *testing.T) {
	home := t.TempDir()
	binPath := filepath.Join(home, ".velgate", "velgate")

	// authorized_keys with the matching line but no velgate dir.
	seedAuthorizedKeys(t, home, sshgateRestrictedLineFor(binPath)+"\n")

	res, err := velgate.Revoke(home, filepath.Join(home, ".velgate"), binPath)
	if err != nil {
		t.Fatalf("Revoke: %v", err)
	}
	if res.LinesRemoved != 1 {
	...
	if res.VelgateDirRemoved {
		t.Errorf("VelgateDirRemoved = true; want false (dir was already absent)")
	}
}
...
func TestRevoke_MultipleMatchingLinesAllRemoved(t *testing.T) {
	...
	// Two restricted entries for the same velgate binary (e.g. multiple
	...
	if strings.Contains(string(got), `"`+binPath+`"`) {
		t.Errorf("rewritten file still contains velgate command directive:\n%s", got)
	}
}

func TestRevoke_MatchesTildeFormWhenBinaryUnderHome(t *testing.T) {
	// The installer writes command="~/.velgate/velgate" but the running
	// velgate sees os.Executable() as the absolute path. Revoke must
	// match both. We seed an authorized_keys with the tilde form and
	// pass the absolute path to Revoke.
	home := t.TempDir()
	binPath := filepath.Join(home, ".velgate", "velgate")

	tildeLine := `command="~/.velgate/velgate",no-port-forwarding,no-X11-forwarding,no-agent-forwarding ssh-ed25519 AAAAtilde sshgate@laptop`
...
func TestRevoke_RejectsEmptyArgs(t *testing.T) {
	if _, err := velgate.Revoke("", "/tmp", "/tmp/velgate"); err == nil {
		t.Error("empty homeDir: expected error")
	}
	if _, err := velgate.Revoke("/tmp", "", "/tmp/velgate"); err == nil {
		t.Error("empty velgateDir: expected error")
	}
	if _, err := velgate.Revoke("/tmp", "/tmp/.velgate", ""); err == nil {
		t.Error("empty velgateBinaryPath: expected error")
	}
}

func TestFormatRevokeStdout(t *testing.T) {
	got := velgate.FormatRevokeStdout(velgate.RevokeResult{LinesRemoved: 1, VelgateDirRemoved: true})
	if !strings.HasPrefix(got, "VELGATE_REVOKED") {
		t.Errorf("got %q; want VELGATE_REVOKED prefix", got)
	}
