...
// when velgate creates the pre-revoke backup. Operators can recover
...
type RevokeResult struct {
	...
	// because their command="..." directive named velgateBinaryPath.
	LinesRemoved int
	// BackupPath is the absolute path to the pre-revoke
	// authorized_keys backup (empty if no backup was needed because
	// no lines matched).
	BackupPath string
	// VelgateDirRemoved is true iff velgateDir existed and was removed
	// without error.
	VelgateDirRemoved bool
}
...
//     command="<velgateBinaryPath>" directive (the spec installer
//     writes the tilde-form "~/.velgate/velgate"; the binary at runtime
...
//  3. Remove the velgate install directory (velgateDir).
//
// The rewrite is atomic: tmp + fsync + rename + fsync(parent). Mode
// 0600 is enforced on the new file.
//
// Revoke does not return an error if the velgate directory is already
// gone, but it does report it via VelgateDirRemoved=false so the caller
// can tell. Missing authorized_keys is also tolerated — there is
// simply nothing to strip.
func Revoke(homeDir, velgateDir, velgateBinaryPath string) (RevokeResult, error) {
	if homeDir == "" {
		return RevokeResult{}, errors.New("revoke: homeDir is empty")
	}
	if velgateDir == "" {
		return RevokeResult{}, errors.New("revoke: velgateDir is empty")
	}
	if velgateBinaryPath == "" {
		return RevokeResult{}, errors.New("revoke: velgateBinaryPath is empty")
	}

	authPath := filepath.Join(homeDir, ".ssh", "authorized_keys")
	res := RevokeResult{AuthorizedKeysPath: authPath}

	candidates := velgateBinaryCandidates(homeDir, velgateBinaryPath)
	...
	if errors.Is(err, fs.ErrNotExist) {
		...
		rewritten, removed := stripVelgateLines(body, candidates)
	...
	// Remove ~/.velgate/. We tolerate ENOENT so a double-revoke is
	// idempotent rather than an error.
	if _, err := os.Stat(velgateDir); err == nil {
		if err := os.RemoveAll(velgateDir); err != nil {
			return res, fmt.Errorf("revoke: remove %s: %w", velgateDir, err)
		}
		res.VelgateDirRemoved = true
	} else if !errors.Is(err, fs.ErrNotExist) {
		return res, fmt.Errorf("revoke: stat %s: %w", velgateDir, err)
	}
	return res, nil
}

// stripVelgateLines returns body with every line whose options include
// a command="<candidate>" directive (for any candidate in
// velgateBinaryCandidates output) removed. removed is the count of
...
// just for revoke.
func stripVelgateLines(body []byte, candidates []string) ([]byte, int) {
...
// velgateBinaryCandidates returns the set of strings that may appear
// inside the command="..." directive of the SSHGate-restricted line in
// authorized_keys. The installer writes the spec's tilde form
// (~/.velgate/velgate) but the running binary sees its resolved
...
func velgateBinaryCandidates(homeDir, velgateBinaryPath string) []string {
	out := []string{velgateBinaryPath}
	if homeDir == "" || velgateBinaryPath == "" {
		return out
	}
	// If the binary lives under $HOME, also try the ~-prefixed form.
	rel, err := filepath.Rel(homeDir, velgateBinaryPath)
...
// FormatRevokeStdout renders the single line velgate emits on stdout
// after a successful revoke. The MCP side detects "VELGATE_REVOKED" as
// the success marker; the trailing details are informational.
func FormatRevokeStdout(res RevokeResult) string {
	parts := []string{"VELGATE_REVOKED"}
	if res.LinesRemoved > 0 {
		parts = append(parts, fmt.Sprintf("lines=%d", res.LinesRemoved))
	}
	if res.VelgateDirRemoved {
