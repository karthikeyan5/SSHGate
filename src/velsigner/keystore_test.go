...
func TestLoadKey_ValidPrivateKey(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "velgate.key")
	...
	got, err := velsigner.LoadKey(path)
	if err != nil {
...
func TestLoadKey_MissingFile(t *testing.T) {
	t.Parallel()
	_, err := velsigner.LoadKey(filepath.Join(t.TempDir(), "nope.key"))
	if err == nil {
...
func TestGenerateKeyPair_WritesUsableKeys(t *testing.T) {
	...
	// Material match
	priv, err := velsigner.LoadKey(privPath)
	if err != nil {
...
func TestGenerateKeyPair_RefusesOverwrite(t *testing.T) {
	...
	err := velsigner.GenerateKeyPair(privPath, pubPath)
	if err == nil {
	...
	if err := velsigner.GenerateKeyPair(priv2, pub2); err == nil {
...
func TestGenerateKeyPair_MissingParentDir(t *testing.T) {
	...
	bad := filepath.Join(t.TempDir(), "nope", "velgate.key")
	err := velsigner.GenerateKeyPair(bad, bad+".pub")
	if err == nil {
