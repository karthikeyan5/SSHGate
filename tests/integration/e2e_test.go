...
// SSH target, real velgate binary on the remote, real velsigner
...
//     SSHGate key and run "rm /tmp/x" without a VELGATE_SIG prefix.
//     velgate exits 77 ("write commands require a VELGATE_SIG
//     prefix"). This is the defense-in-depth check: even if an
//     attacker had the SSHGate key, velgate stops them.
...
func TestPhase1EndToEnd(t *testing.T) {
	...
	// The master key path: re-derive — generateVelgateKeyPair returns
	// (priv, pub); we need priv for velsigner.
	masterKey := strings.TrimSuffix(velgatePub, ".pub") + ".key"

	// Stand up the stack once for all four scenarios. The container
	// + velsigner are expensive (~5s); reusing them keeps the suite
	...
	socketPath, velsignerCleanup := startVelsigner(t, masterKey)
	t.Cleanup(velsignerCleanup)
	...
	t.Run("DirectUnsignedSSHBypassRefused", func(t *testing.T) {
		...
		// dedicated key works (OpenSSH accepts it), but velgate's
		// command="..." forcing intercepts, classifies "rm /tmp/x"
		// as a write, sees no VELGATE_SIG prefix, exits 77.
		...
		if exit != 77 {
			t.Errorf("exit=%d; want 77 (EX_NOPERM from velgate)", exit)
		}
		if !strings.Contains(string(stderr), "write commands require a VELGATE_SIG prefix") {
			t.Errorf("stderr did not carry the expected velgate message: %q", stderr)
		}
	...
	t.Run("NoGoroutineLeaks", func(t *testing.T) {
		...
		// velsigner are still registered, so we run goleak BEFORE
		...
		// returns, and velsigner.Server's accept loop exits when
		// its ctx is cancelled by velsignerCleanup.
		goleak.VerifyNone(t,
